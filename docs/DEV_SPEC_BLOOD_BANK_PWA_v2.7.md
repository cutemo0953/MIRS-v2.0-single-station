...
## v2.8.2 變更摘要 (2026-01-14)

### 色系統一 (MIRS Blood 風格)

全面採用 MIRS 血袋管理 Tab 的 **紅色系** (`red-700`, `red-600`, `red-200`)，取代原本的灰色/綠色/藍色混用。

| 項目 | 舊色系 | 新色系 |
|------|--------|--------|
| **發血流程按鈕** | `bg-gray-800` | `bg-red-700` |
| **查詢病患位置** | `bg-gray-800` | `bg-red-700` |
| **掃描血袋條碼** | `bg-gray-800` | `bg-red-700` |
| **確認發血按鈕** | `bg-gray-800` | `bg-red-700` |
| **待補單完成按鈕** | `bg-green-600` | `bg-red-700` |
| **病患位置顯示** | `border-gray-800` | `border-red-600` + `bg-red-50` |
| **輸入框 focus** | `ring-gray-500` | `ring-red-500` |

### Heroicon 替換

| 項目 | 舊 | 新 |
|------|-----|-----|
| **捐血中心入庫 Modal** | 🏥 emoji | 建築物 heroicon |

### 設計原則

```
┌─────────────────────────────────────────────────────────────────┐
│  MIRS Blood Bank PWA 色系規範                                    │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  主要按鈕:     bg-red-700  hover:bg-red-800                       │
│  緊急按鈕:     bg-red-600  hover:bg-red-700                       │
│  次要按鈕:     bg-gray-100 text-gray-700                          │
│  卡片邊框:     border-red-200                                     │
│  選中狀態:     border-red-600 bg-red-50                           │
│  文字強調:     text-red-700                                       │
│  focus ring:   ring-red-500                                       │
│                                                                   │
│  禁止使用:     green-*, blue-*, amber-* (僅警示訊息例外)          │
│                                                                   │
└─────────────────────────────────────────────────────────────────┘
```
...
| 檔案 | 變更 |
|------|------|
| `frontend/blood/index.html` | 發血/待補單色系統一、捐血中心 Modal heroicon |
| `frontend/blood/service-worker.js` | v2.8.2 快取刷新 |

---

## v2.8.1 變更摘要 (2026-01-14)

### UI 優化

| 項目 | 變更 |
|------|------|
| **標籤補印** | 改為 Client-side 生成 HTML 標籤，不再呼叫 API |
| **緊急捐血 Modal** | 加入半透明 backdrop、改良關閉按鈕、移除漸層改單色 (orange-600) |
| **緊急領血 FAB** | emoji 改 heroicon (警告三角形) |
| **發血作業 UI** | 各欄位加 icon |
...
**新架構:**
1. **庫存總覽** - 統計卡片 + **完整單位列表** (含 FIFO/預約/發血按鈕)
2. **入庫+標籤** - 入庫來源選擇 + 緊急捐血 + 補印標籤
3. 發血 - 不變
4. 待補單 - 不變
...
| 項目 | 說明 |
|------|------|
| **庫存總覽整合** | 點擊血型卡片後，直接在總覽 Tab 顯示完整血袋清單，可直接發血/預約 |
| **入庫來源選擇** | 卡片式選擇：捐血中心 vs 緊急捐血 (Walking Blood Bank) |
| **緊急捐血流程** | 新增捐血者資訊記錄 (姓名/電話)，自動設定 7 天效期 |
| **補印標籤** | 入庫+標籤 Tab 可從現有血袋清單補印標籤 |
...
│  │    🏥          │  │    🚨          │                          │
│  │   捐血中心      │  │   緊急捐血      │                          │
│  │  血液基金會配送  │  │ Walking Blood  │                          │
│  │               │  │     Bank       │                          │
│  └────────────────┘  └────────────────┘                          │
│                                                                   │
│  ┌─────────────────────────────────────────────────────────────┐ │
│  │  🏷️ 補印標籤                                                │ │
│  │  O+ [PRBC] BU-20260113...              [列印]               │ │
│  │  O- [WB]   BU-20260112...              [列印]               │ │
│  │  A+ [FFP]  BU-20260111...              [列印]               │ │
│  └─────────────────────────────────────────────────────────────┘ │
│                                                                   │
│  ┌─────────────────────────────────────────────────────────────┐ │
│  │  ⏰ 最近入庫                                                 │ │
...
```
┌─────────────────────────────────────────────────────────────────┐
│  🚨 緊急捐血 (Walking Blood Bank)                     [X]        │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  ⚠️ 緊急捐血：適用於現場捐血者直接輸血的緊急狀況。                 │
│     需記錄捐血者資訊，事後補做完整血液檢驗。                       │
│                                                                   │
│  ┌─────────────────────────────────────────────────────────────┐ │
│  │  捐血者資訊                                                  │ │
│  │  姓名 *: [____________]   電話: [____________]              │ │
│  └─────────────────────────────────────────────────────────────┘ │
│                                                                   │
│  血型 *: [O+ ▼]              血品類型: [WB (全血) - 緊急首選]     │
│                                                                   │
│  ℹ️ 效期將自動設為 7 天 (新鮮全血凝血因子有效期)                  │
│                                                                   │
│  [              確認緊急入庫              ]                       │
│                                                                   │
└─────────────────────────────────────────────────────────────────┘
...
| v2.8.0 | 2026-01-14 | Tab 重構: 庫存總覽整合清單、入庫+標籤 Tab、緊急捐血流程 |
