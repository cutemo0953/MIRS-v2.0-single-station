...
### v3.2 主要更新 (2026-01-06)

1. **Step 0 完整欄位**：目的地、ETA、總時間、病患狀況、O2、IV、安全係數、EMT 姓名
2. **CIRS Handoff 整合**：接收 CIRS 交班單，自動建立任務
3. **智慧返回鍵**：左箭號返回上一步（非回首頁）
4. **物資自動重算**：Step 0 設定變更後自動重新計算物資需求
5. **PATCH API**：`PATCH /api/transfer/missions/{id}` 支援所有欄位更新
6. **ISBAR/MIST 格式**：手動建立支援兩種交班格式
